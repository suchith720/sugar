...
def extract_metadata(outputs:Dict, lbl_ids:List):
    ...
    assert len(str_ids) == lbl_str.shape[1], (
        f"Concept count mismatch: expected {lbl_str.shape[1]}, got {len(str_ids)}"
    )
...
if __name__ == "__main__":
    ...
    if os.path.exists(out_file):
        outputs = joblib.load(out_file)
    else:
        df = combine_df(output_dir)
        outputs = convert_string_to_object(df)
        joblib.dump(outputs, out_file)
